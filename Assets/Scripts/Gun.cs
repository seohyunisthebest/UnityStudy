...
public class Gun : MonoBehaviour {
    ...
    private void Shot() {
        ...
        if (Physics.Raycast(fireTransform.position, fireTransform.forward, out hit, fireDistance))
        {
            ...
            // 충동한 상대방으로부터 IDamageable 오브젝트 가져오기 시도
            IDamageable targert = hit.collider.gameObject.GetComponent<IDamageable>();

            // 상대방으로부터 Idamageble 오브젝트를 가져오는 데 성공했다면
            if (targert != null )
            {
                // 상대방의 OnDamage 함수를 실행시켜 상대방에 데미지 주기
                targert.OnDamage(damage, hit.point, hit.normal);
            }
            // 레이가 충돌한 위치 저장
    ...
    private IEnumerator ReloadRoutine() {
        ...
        // 탄창에 채울 탄알을 계산
        int ammoToFil = magCapacity - magAmmo;
